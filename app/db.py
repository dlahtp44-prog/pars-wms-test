...
def init_db() -> None:
    ...
    # Calendar memos (shared)
    cur.execute("""CREATE TABLE IF NOT EXISTS calendar_memos(
...
def upsert_calendar_memo(date: str, memo: str, author: str = "") -> None:
    """Upsert a memo for a given date (YYYY-MM-DD)."""
    conn = get_db()
    cur = conn.cursor()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    cur.execute(
        """INSERT INTO calendar_memos(date, memo, author, updated_at)
           VALUES(?, ?, ?, ?)
        ...
        """,
        (date, memo, author, now),
    )
    conn.commit()
    conn.close()


def get_calendar_memos_for_month(year: int, month: int) -> dict:
    """Return dict {date: {memo, author, updated_at}} for a month."""
    ...
    cur = conn.cursor()
    cur.execute(
        """SELECT date, memo, author, updated_at
             FROM calendar_memos
             WHERE date >= ? AND date < ?
             ORDER BY date ASC""",
        (start_date, end_date),
    ...
    rows = cur.fetchall()
    conn.close()
    return {r[0]: {"memo": r[1], "author": r[2], "updated_at": r[3]} for r in rows}
