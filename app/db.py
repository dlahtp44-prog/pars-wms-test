...
def init_db() -> None:
    ...
    # Calendar memo table (shared)
    cur.execute("""
    CREATE TABLE IF NOT EXISTS calendar_memo (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        memo_date TEXT NOT NULL UNIQUE,   -- YYYY-MM-DD
...
# -----------------------------
# Calendar memo helpers
# -----------------------------
def get_calendar_month(year: int, month: int):
    """Return dict date->memo for given month (YYYY-MM)."""
    ym = f"{year:04d}-{month:02d}"
    conn = get_db()
    cur = conn.cursor()
    cur.execute(
        "SELECT memo_date, memo_text FROM calendar_memo WHERE memo_date LIKE ?",
        (ym + "%",),
    )
    rows = cur.fetchall()
    conn.close()
...
def save_calendar_memo(memo_date: str, memo_text: str):
    """Upsert memo by date (YYYY-MM-DD). Empty text deletes memo."""
    conn = get_db()
    cur = conn.cursor()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    if memo_text is None or str(memo_text).strip() == "":
        cur.execute("DELETE FROM calendar_memo WHERE memo_date = ?", (memo_date,))
        conn.commit()
        conn.close()
        return {"deleted": True}
    cur.execute(
        """INSERT INTO calendar_memo (memo_date, memo_text, updated_at)
             VALUES (?, ?, ?)
             ON CONFLICT(memo_date) DO UPDATE SET memo_text=excluded.memo_text, updated_at=excluded.updated_at
        """,
        (memo_date, memo_text, now),
    )
    conn.commit()
    conn.close()
    return {"ok": True}
